...
public class MailboxManager {
    ...
    static void loginToServer() throws Exception {
        ...
        sendCommand("STAT");
        // Read the server's reply to STAT which contains 
        // the number of messages and total bytes
    ...
    static void loadHeadersForMessage(int messageNumber) throws Exception {
        ...
        // Default values in case the headers are missing
        String from    = "(unknown sender)";
        String subject = "(no subject)";

        // Read lines until we see a single dot "." which means end of message
    ...
    static void searchMessages() {
        ...
        int matchCount = 0;
        // Check if the keyword appears in either the sender or subject of each message
        for (int i = 0; i < messageNumbers.size(); i++) {
            // We convert both the keyword and the text to lowercase so the search is case-insensitive
            ...
            // If either the sender or subject matches, we show this message in the results
